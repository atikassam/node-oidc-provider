...
module.exports = async function resumeAction(whitelist, resumeRouteName, ctx, next) {
  ...
  if (
    ...
    ctx.res.redirect(ctx.oidc.urlFor(ctx.oidc.route, ctx.params));
    // session.state = {
    //   secret: nanoid(),
  ...
  if (result && result.select_account && result.select_account.id) {
    const { id } = result.select_account;
    const sessions = JSON.parse(ctx.cookies.get('_sessions') || '[]');
    ...
    const { maxAge: m, ...opts } = instance(ctx.oidc.provider).configuration('cookies.long');
