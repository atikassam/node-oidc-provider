...
class Provider extends events.EventEmitter {
   ...
   * @api public
   */
  async switchProviderSession(req, res, {
    id,
  } = {}) {
    assertReqRes(req, res);
    ...
    const ctx = this.app.createContext(req, res);
    const interaction = this.interactionDetails(req, res);

    const session = await this.Session.get(ctx);
    const sessions = JSON.parse(ctx.cookies.get('_sessions') || '[]');

    const switchSession = sessions.find(({ id: jti }) => jti === id);
    if (!switchSession) throw new Error('No such session is available');

    const sessionDetails = await session.adapter.find(switchSession.id);
    if (!switchSession || !sessionDetails) throw new Error('No such session is available');
    ...
    ssHandler.set(
      ...
      this.cookieName('session'),
      switchSession.id,
      switchSession.transient ? opts : { maxAge, ...opts },
    );

    sessions.splice(sessions.indexOf(switchSession), 1);
    sessions.push({
      id: session.id,
    ...
    interaction.session = {
      accountId: sessionDetails.account,
      ...(sessionDetails.uid ? { uid: sessionDetails.uid } : undefined),
      ...(sessionDetails.jti ? { cookie: sessionDetails.jti } : undefined),
      ...(sessionDetails.acr ? { acr: sessionDetails.acr } : undefined),
      ...(sessionDetails.amr ? { amr: sessionDetails.amr } : undefined),
    };

    await interaction.save();
    ctx.cookies.set('_sessions', JSON.stringify(sessions));
    return session;
  }
