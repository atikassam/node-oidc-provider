...
class Provider extends events.EventEmitter {
  ...
  async switchProviderSession(req, res, {
    ...
    const ssessionDetails = await session.adapter.find(switchSession.id);

    if (!switchSession || !ssessionDetails) throw new Error('No such session is available');
    ...
    interaction.session = {
      accountId: ssessionDetails.account,
      ...(ssessionDetails.uid ? { uid: ssessionDetails.uid } : undefined),
      ...(ssessionDetails.jti ? { cookie: ssessionDetails.jti } : undefined),
      ...(ssessionDetails.acr ? { acr: ssessionDetails.acr } : undefined),
      ...(ssessionDetails.amr ? { amr: ssessionDetails.amr } : undefined),
    };
